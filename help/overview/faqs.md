...
Votre version d’AEM Cloud Manager échoue lors de la tentative de basculement de Java 8 vers Java 11. Si vous rencontrez l’erreur ci-dessous, vous devez supprimer `maven-scr-plugin` et convertir toutes les annotations OSGi en annotations OSGi R6.
...
Ce problème se produit, car Cloud Manager utilise une autre version de Java pour exécuter la commande Maven. Cette version est différente de celle utilisée pour compiler le code. Omettez `requireJavaVersion` de vos configurations `maven-enforcer-plugin`.

## La vérification de la qualité du code a échoué et le déploiement est maintenant interrompu. Y a-t-il un moyen de contourner cette vérification ? {#deployment-stuck}
...
## Les déploiements de Cloud Manager échouent à l’étape de test de performances dans les environnements Managed Services d’Adobe. Comment résoudre ce problème pour transmettre les mesures critiques ? {#debug-critical-metrics}

Il n’existe pas de réponse définitive à cette question. Toutefois, les points suivants concernant l’étape du test de performance sont utiles :

* Cette étape est une étape de performances web. En d’autres termes, il s’agit du temps nécessaire pour charger une page dans un navigateur web.
* Les URL répertoriées dans le fichier .csv de résultat se chargent dans un navigateur Chrome au sein de l’infrastructure Cloud Manager pendant le test.
...
Cela permet aux déploiements suivants de s’installer lorsque la version n’a pas changé. Pour les déploiements développeurs, aucune version automatique n’est ajoutée ou générée pour la version Maven.
...
## Mon build Maven échoue pour les déploiements Cloud Manager, mais elle est créée localement sans erreur. Quelle en est la cause ? {#maven-build-fail}
...
Vous recevez une erreur 403 telle que la suivante lorsque vous tentez de répertorier ou de définir des variables de pipeline au moyen de commandes `aio`.
