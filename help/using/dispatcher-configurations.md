...
## Utilisation de Cloud Manager pour déployer les fichiers de configuration du Dispatcher {#using-cloud-manager-to-deploy-dispatcher-configuration-files}
...
Pour tirer parti de cette fonctionnalité, la compilation Maven doit produire un fichier zip contenant au moins deux répertoires : ***conf*** et ***conf.d***. Ce fichier zip peut être créé à l’aide de maven-assembly-plugin. Les projets générés par Cloud Manager à l’aide de l’[assistant](create-an-application-project.md) intégré ont une structure de projet Maven correcte déjà créée dans le cadre de la création du projet.

Lors du déploiement sur une **Instance** de Dispatcher, le contenu de ces répertoires remplacera celui de ces répertoires sur l’instance de Dispatcher. Comme les fichiers de configuration du serveur web et du Dispatcher requièrent souvent des informations spécifiques à l’environnement, vous devez d’abord collaborer avec les ingénieurs du service client pour extraire ces variables d’environnement dans ***/etc/sysconfig/httpd***. [](create-an-application-project.md)

### Procédure de configuration du Dispatcher {#steps-for-configuring-dispatcher}
...
Le fichier et la structure de répertoires spécifiques peuvent varier en fonction des détails de votre projet, mais cet exemple doit fournir un guide concret sur la structure de votre projet pour inclure les configurations d’Apache et du Dispatcher.
...
1. Créez maintenant un sous-répertoire appelé `src` (comme référencé dans le descripteur d’assemblage ci-dessus à la ligne 11) dans le sous-répertoire dispatcher pour stocker les configurations Apache et Dispatcher actuelles. Dans ce répertoire `src`, créez des répertoires appelés `conf`, `conf.d`, `conf.dispatcher.d` et `conf.modules.d`.
1. Vous pouvez maintenant remplir les répertoires `conf`, `conf.d`, `conf.dispatcher.d` et `conf.modules.d` avec vos fichiers de configuration. Par exemple, la configuration par défaut comprend ces fichiers et liens symboliques.
...
1. Enfin, dans le fichier pom.xml à la racine de votre projet, ajoutez un élément `<module>` pour inclure le module dispatcher.
