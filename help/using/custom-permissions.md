...
La création et l’utilisation de vos propres autorisations personnalisées nécessitent les trois étapes suivantes :
...
>[!NOTE]
>
>Vous devez disposer des droits d’administrateur de produit dans l’Admin Console pour créer de nouveaux profils et gérer les autorisations pour Cloud Manager.
...
1. Vous êtes redirigé vers l’onglet **Produits** de l’Admin Console, où vous pouvez gérer les utilisateurs et les autorisations pour Cloud Manager. Dans l’Admin Console, cliquez sur **Nouveau profil**.
...
1. Fournissez les détails généraux du profil.
   ...
   * **Avertir les utilisateurs par e-mail** - Lorsque cette option est sélectionnée, le système avertit les utilisateurs par e-mail lorsqu’ils sont ajoutés ou supprimés de ce profil.
...
1. Cliquez sur l’icône plus (`+`) en regard de l’élément d’autorisation pour l’ajouter à la colonne **Éléments d’autorisation inclus**. Si nécessaire, cliquez sur l’icône `i` en regard d’un élément d’autorisation pour en savoir plus.
...
1. Lorsque vous avez terminé de définir les éléments d’autorisation pour votre nouveau profil de produit, cliquez sur **Enregistrer**.

Votre nouveau profil de produit est maintenant enregistré avec ses autorisations personnalisées.

### Affecter des utilisateurs au nouveau profil de produit {#assign-users}
...
1. Cliquez sur **Ajouter des utilisateurs** et affectez des utilisateurs à votre nouveau profil de produit avec des autorisations personnalisées.

Pour plus d’informations sur l’utilisation de l’Admin Console, voir **Ajout d’utilisateurs et de groupes d’utilisateurs à un profil de produit** du document [Gestion des profils de produit pour les utilisateurs d’entreprise](https://helpx.adobe.com/fr/enterprise/using/manage-product-profiles.html).
...
**Repository Info Access** est un exemple d’autorisation au niveau de l’organisation dans Cloud Manager. Cette autorisation permet aux utilisateurs de générer un nom d’utilisateur, un mot de passe et une URL de référentiel pour accéder aux projets des clients et y contribuer. Bien que le nom d’utilisateur et le mot de passe restent cohérents dans tous les référentiels de l’organisation, chaque programme dispose d’une URL de référentiel unique.

Pour plus d’informations, consultez le [référentiel de code Source](/help/requirements/source-code-repository.md) .
...
| Autorisations personnalisées | Fonctionnalités Cloud Manager qui permettent aux utilisateurs de créer des profils d’autorisation pour définir des rôles pour régir les fonctionnalités prises en charge de Cloud Manager |
| Profil d’autorisation | Créé dans l’Admin Console pour gérer les autorisations configurables applicables aux utilisateurs faisant partie du profil d’autorisation. |
| Autorisation configurable | Les autorisations Cloud Manager peuvent être configurées dans le profil d’autorisation. |
...
Les éléments d’autorisation se rapportent à la portée dans laquelle les autorisations sont appliquées. En règle générale, il s’agit de l’un des suivants.
...
* Dans de rares cas où un service d’autorisations personnalisé ne répond pas, les profils prédéfinis sont toujours disponibles et les utilisateurs des profils prédéfinis disposent toujours d’un accès approprié.
...
Les profils de produit et les rôles Cloud Manager par défaut fonctionnent toujours de la même manière qu’auparavant.
...
Dans ce scénario, l’utilisateur ne peut pas accéder à l’environnement ou au pipeline s’il ne dispose pas des autorisations **Accès au programme** contenant l’environnement ou le pipeline.
...
Créez un profil distinct pour chaque type de produit. C’est-à-dire, un pour AEM en tant que Cloud Service et un pour Adobe Managed Services ou AMS.
