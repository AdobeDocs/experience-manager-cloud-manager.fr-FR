...
>[!NOTE]
>
>Vous devez disposer des droits d’administrateur de produit dans Admin Console pour créer de nouveaux profils et gérer les autorisations de Cloud Manager.
...
1. Sélectionner le produit **Managed Services AEM**.

1. Recherchez et l’instance dont le nom correspond au modèle. `*-cloud-manager` et appuyez ou cliquez sur pour gérer les utilisateurs et les autorisations.
...
1. Appuyez ou cliquez sur le bouton **Tout ajouter** en haut de la page **Autorisations disponibles** pour ajouter toutes les autorisations. De même, appuyez ou cliquez sur **Tout supprimer** pour supprimer toutes les autorisations sélectionnées précédemment.
...
| Profil d’autorisation | Créé dans la console d’administration pour gérer les autorisations configurables qui s’appliqueront aux utilisateurs faisant partie du profil d’autorisation. |
...
* A [un ensemble limité d’autorisations est disponible](#configurable-permissions) pour créer des profils personnalisés.
...
### Que se passe-t-il si j’ai AEM programmes as a Cloud Service et AMS dans la même organisation IMS ? Puis-je gérer les autorisations d’un profil ? {#ams-and-aemaacs}

Vous devez créer un profil distinct pour chaque type de produit (c’est-à-dire un pour AEM en tant que Cloud Service et un pour Adobe Managed Services ou AMS).
